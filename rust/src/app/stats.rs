...
use crate::index::{IndexStore, JobRecord, SourceKind, TaskStatus, TaskType};
use std::path::Path;
...
pub fn collect_stats_overview(repo_root: &Path) -> Result<StatsOverview, String> {
    ...
    for (stage, label) in stage_definitions() {
        ...
        for job in &upload_jobs {
            match classify_stage_bucket(&store, job, stage)? {
...
fn classify_stage_bucket(
    ...
    Ok(match job.task(stage).map(|task| task.status) {
        Some(TaskStatus::Queued | TaskStatus::Running) => StageBucket::InProgress,
        Some(TaskStatus::Failed) => StageBucket::Excluded,
        Some(TaskStatus::Completed) | None => StageBucket::Unprocessed,
    })
}
