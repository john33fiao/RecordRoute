use super::{
    ...
    ffmpeg_stage, now_rfc3339, record_previous_audio_keys, refresh_audio_cache, stt_stage,
    summary_stage, sync_audio_cache_dispatch_state,
};
...
fn dispatch_one_internal(
    ...
    let mut predicted_dispatch_state = state.clone();
    predicted_dispatch_state.record_execution(work.entry.category);
    sync_audio_cache_dispatch_state(repo_root, &predicted_dispatch_state);
    refresh_audio_cache(repo_root);

    let execution_result = execute_work_item(repo_root, &work.entry);
    if let Err(error) = record_previous_audio_keys(repo_root, &work.entry) {
        eprintln!("{error}");
    }
    ...
    sync_audio_cache_dispatch_state(repo_root, state);
    refresh_audio_cache(repo_root);
    ...
    match &entry.payload {
        ...
        QueuePayload::Stt {
            ...
            let paths = if audio_files.is_empty() {
                artifacts::listed_or_discovered_audio_files(
