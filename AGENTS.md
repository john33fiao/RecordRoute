...
- `/api/similarity-graph`, `/api/documents/metadata`
  - `/api/similarity-graph` 응답 `meta`는 `sampling` + `incremental(reused_pairs/computed_pairs/added_docs/removed_docs/changed_docs)` + `cache(hit/refresh/ttl/max_entries)` 진단 정보를 포함
