...
- `/api/similarity-graph`, `/api/documents/metadata`
  ...
  - `/api/similarity-graph` 응답 `meta`는 `sampling` + `incremental(reused_pairs/computed_pairs/added_docs/removed_docs/changed_docs)` + `candidate_reduction(strategy/total_pairs/candidate_pairs/reduction_ratio/estimated_recall_at_k)` 진단 정보를 포함
