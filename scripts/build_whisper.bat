...
set "built_whisper_bin=%backend_build_dir%\bin\whisper-cli.exe"
set "built_whisper_release_bin=%backend_build_dir%\bin\Release\whisper-cli.exe"
if not exist "%runtime_bin%" mkdir "%runtime_bin%"
if exist "%built_whisper_bin%" (
  copy /y "%built_whisper_bin%" "%whisper_bin%" >nul
)
if exist "%whisper_bin%" (
  echo whisper_cli=%whisper_bin%
  exit /b 0
)
if exist "%built_whisper_release_bin%" (
  copy /y "%built_whisper_release_bin%" "%whisper_bin%" >nul
)
if exist "%whisper_bin%" (
  echo whisper_cli=%whisper_bin%
  exit /b 0
...
set "built_whisper_bin=%backend_runtime_bin%\whisper-cli.exe"
set "built_whisper_release_bin=%backend_runtime_bin%\Release\whisper-cli.exe"
...
"%cmake_exe%" --build "%backend_build_dir%" --target whisper-cli --parallel %jobs%
if errorlevel 1 exit /b %errorlevel%

if exist "%built_whisper_bin%" copy /y "%built_whisper_bin%" "%whisper_bin%" >nul
if exist "%built_whisper_release_bin%" copy /y "%built_whisper_release_bin%" "%whisper_bin%" >nul

if not exist "%whisper_bin%" (
  >&2 echo whisper-cli binary not found after build: %built_whisper_bin%
  exit /b 1
)
...
set "source_dir=%repo_root%\modules\whisper.cpp"

if not exist "%source_dir%" (
  >&2 echo whisper.cpp source directory not found: %source_dir%
  exit /b 1
)
...
set "build_root=%repo_root%\.build\whisper\%target%"
set "runtime_bin=%build_root%\bin"
set "whisper_bin=%runtime_bin%\whisper-cli.exe"
...
if defined cached_backend if exist "%whisper_bin%" (
  echo whisper_cli=%whisper_bin%
  exit /b 0
)
...
>&2 echo failed to build whisper-cli with supported backends: cuda cpu
exit /b 1
