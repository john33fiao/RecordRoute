...
export function SimilarDocsDialog({ open, onOpenChange, filePath, filename }: SimilarDocsDialogProps) {
  ...
  const [selectedDoc, setSelectedDoc] = useState<SimilarDocument | null>(null);
  ...
  const handleDocClick = (doc: SimilarDocument) => {
    setSelectedDoc(doc);
    setShowOverlay(true);
  };
    ...
    <>
          ...
          <ScrollArea className="h-[500px] pr-4">
            {loading ? (
              ...
              <div className="space-y-3">
                {docs.map((doc, i) => (
                  <div key={i} onClick={() => handleDocClick(doc)}
      ...
      {selectedDoc && (
        <TextOverlay
          ...
          fileIdentifier={selectedDoc.file_uuid || selectedDoc.file}
          fileType="stt"
          filename={selectedDoc.display_name || selectedDoc.source_filename || selectedDoc.file}
          recordId={selectedDoc.record_id || ''}
        />
      )}
    </>
  );
